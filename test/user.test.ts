...
describe("user api", () => {
  ...
  it("sends a request to delete user account", async () => {
    const { data } = await holipoly.user.accountRequestDeletion(API_URI);
    expect(data?.accountRequestDeletion?.errors).toHaveLength(0);
  });

  it("sends request to change user email", async () => {
    const { data } = await holipoly.user.requestEmailChange({
  ...
  it("updates the user first name", async () => {
    const { data } = await holipoly.user.updateAccount({
      input: {
        firstName: "",
      },
    });
  ...
  it("creates the user account address", async () => {
    const { data } = await holipoly.user.createAccountAddress({
      input: testAddress,
    });
  ...
  it.skip("updates the user account address", async () => {
    const { data: newAddress } = await holipoly.user.createAccountAddress({
      input: testAddress,
    });
    ...
    if (addressId) {
      ...
      const { data } = await holipoly.user.updateAccountAddress({
        id: addressId,
        input: {
          ...testAddress,
  ...
  it("sets address as a default billing address", async () => {
    ...
    if (addressId) {
      ...
      const { data } = await holipoly.user.setAccountDefaultAddress({
        id: addressId,
        type: "BILLING",
      });
      expect(
      ...
      if (oldAddress) {
        const {
          data: revertedData,
        } = await holipoly.user.setAccountDefaultAddress({
          id: oldAddress.id,
          type: "BILLING",
        });
        expect(
  ...
  it.skip("deletes user address", async () => {
    ...
    expect(addressId).toBeTruthy();
    if (addressId) {
      const { data } = await holipoly.user.deleteAccountAddress(addressId);
      const state = holipoly.getState();
