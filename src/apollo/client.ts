...
export type FetchConfig = Partial<{
  ...
  /**
   * Refresh token and retry the request when Holipoly responds with `Unauthorized` error.
   * Defaults to `true`.
   */
...
export const createFetch = ({
  ...
  if (!client) {
    throw new Error(
      "Could not find Holipoly's client instance. Did you forget to call createHolipolyClient()?"
    );
  }
    ...
    if (isUnauthenticated) {
      try {
        ...
        if (
          ...
          // after Holipoly returns ExpiredSignatureError status and token refresh fails
