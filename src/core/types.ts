...
export interface HolipolyClientInternals {
  apolloClient: ApolloClient<NormalizedCacheObject>;
}
export interface HolipolyClientConfig {
  channel: string;
  autologin: boolean;
  setChannel(channel: string): string;
}
export interface HolipolyClient {
  auth: AuthSDK;
  user: UserSDK;
  config: HolipolyClientConfig;
  _internal: HolipolyClientInternals;
  getState(): State;
}

interface HolipolyClientFetchOpts {
  autologin?: boolean;
  fetchOpts?: FetchConfig;
}

export interface HolipolyClientOpts {
  apiUrl: string;
  channel: string;
  opts?: HolipolyClientFetchOpts;
}

export type HolipolyClientMethodsProps = HolipolyClientInternals &
  Pick<HolipolyClientConfig, "channel">;
